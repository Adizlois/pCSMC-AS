...
The structure of the scripts is similar in all cases (except for JAGS). Each script begins with a function implementing the method (e.g., PGAS, CSMC, pCSMC), followed by a main function. At the end, the key variables (e.g., number of particles, number of iterations) are set, the main function is called, and the results are stored as an RDS object.

Contact: adpaloma@uio.no
